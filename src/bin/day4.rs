...
impl SleepPattern {
    ...
    fn most_asleep(&self) -> Option<(usize, usize)> {
        ...
        self.by_min.iter().cloned().enumerate().max_by_key(|(_, count)| *count)
    }
}
...
fn part_a(guard_patterns: &HashMap<usize, SleepPattern>) -> usize {
    let (id, _, min) = guard_patterns.iter().map(|(k, v)| {
        let most_asleep = v.most_asleep().unwrap_or((0, 0));
        (k, v.num_mins, most_asleep.0)
    }).max_by_key(|(_, num_mins, _)| *num_mins).unwrap();
...
fn part_b(guard_patterns: &HashMap<usize, SleepPattern>) -> usize {
    ...
    }).max_by_key(|(_, most_sleep, _)| *most_sleep).unwrap();
...
fn main() {
    ...
    for (id, event) in it {
        let entry = guard_patterns.entry(id.unwrap().get()).or_insert(SleepPattern::new());
